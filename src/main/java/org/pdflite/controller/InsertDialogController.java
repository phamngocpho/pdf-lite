...
public class InsertDialogController {
    ...
    // --- ĐÃ SỬA TÊN HÀM DƯỚI ĐÂY ĐỂ KHỚP VỚI MAIN ---

    public int getInsertCount() { // Sửa từ getCount -> getInsertCount
        return countSpinner.getValue();
    }

    public int getInsertIndex(int current, int total) { // Sửa từ getIndex -> getInsertIndex
        if (rbLast.isSelected()) return total;
        if (rbBefore.isSelected()) return current;
        return current + 1;
    }
    // --- THÊM HÀM NÀY ĐỂ FIX LỖI ---
    public int getIndex(int current, int total) {
        return getInsertIndex(current, total);
    }
}